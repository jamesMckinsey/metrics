...
# main metrics __init__ can detect if it is being loaded by the setup
# routine, to avoid attempting to load components that aren't built yet:
# the numpy distutils extensions that are used by scikit-learn to
# recursively build the compiled extensions in sub-packages is based on the
# Python import machinery.
builtins.__metrics_SETUP__ = True
...
# We can actually import a restricted version of metrics that
...
try:
    ...
    class build_ext_subclass(build_ext):
        def build_extensions(self):
            from metrics._build_utils.openmp_helpers import get_openmp_flag

            if metrics._OPENMP_SUPPORTED:
...
def configuration(parent_package='', top_path=None):
    ...
    from metrics._build_utils import _check_cython_version
